...
export function CodeEditor({ code, isStreaming, onCodeChange }: Props) {
  ...
  const download = () => {
    ...
    const a = Object.assign(document.createElement('a'), { href: url, download: 'GeneratedComponent.tsx' });
    ...
    <div className="flex-1 flex flex-col bg-white rounded-lg sm:rounded-xl shadow-lg border border-gray-200 overflow-hidden h-full">
      <div className="flex items-center justify-between px-2 sm:px-4 py-2 sm:py-3 bg-gray-50 border-b border-gray-200 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            onClick={copy}
            disabled={!code}
            className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-xs sm:text-sm font-medium transition-colors disabled:opacity-50"
          >
      ...
      <div className="flex-1 relative min-h-0">
        <textarea
          ...
          className={`w-full h-full p-2 sm:p-3 font-mono text-xs sm:text-sm bg-gray-900 text-gray-100 border-0 resize-none focus:outline-none focus:ring-2 focus:ring-orange-500 ${isStreaming ? 'streaming-cursor' : ''}`}
